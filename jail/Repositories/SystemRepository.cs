...
namespace jail {
    ...
    internal class SystemRepository : BaseRepository {
        ...
        public static IEnumerable<SystemLog> GetErrorLogData(int count, string key = null,
...
Exception, CallerAddress from SystemLogs where 1=1");
            if (!string.IsNullOrWhiteSpace(key))
                sql.Append(
                    " and (Message like @key or Exception like @key or UserName like @key or CallerAddress like @key)");
            switch (searchType) {
            ...
            var result = GetConnection()
                .Query<SystemLog>(sql.ToString(), new {count, key = string.Format("%{0}%", key)});
        ...
        public static int ClearByMessagePart(string selection) {
            return GetConnection().Execute(
                "delete from SystemLogs where Message like '%' || @selection || '%' or UserName=@selection or MachineName=@selection or CallerAddress like @selection",
                new {selection});
        }

        public static long CalcByMessagePart(string selection) {
            return GetConnection().ExecuteScalar<long>(
                "select count(1) from SystemLogs where Message like '%' || @selection || '%' or UserName=@selection or MachineName=@selection or CallerAddress like @selection",
                new {selection});
        }
